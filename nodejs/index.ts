...
const SIGNER_PRIVATE_KEY: Hex = (process.env.SIGNER_PRIVATE_KEY || zeroAddress) as Hex; // Optional, using the default means a new signer will be created each time
...
const HUB_URL = "hub-grpc.pinata.cloud"; // URL + Port of the Hub
...
const USE_SSL = true; // set to true if talking to a hub that uses SSL (3rd party hosted hubs or hubs that require auth)
